...
def get_state(obs):
    ...
    return (isArmyGrows, hasIdleWorkders, canAttack, enemies_dead_count, hasArmy, hasBarracks ), (canSelectWorker, canGather, canBuildBarracks, canTrainScv, canBuildSupplyDepot, canTrainMarine, canSelectArmy), mineralTarget, targetTerranCenter, targetForBuild, targetScv, targetForBuildBaracks, targetBarracks, targetAttackEnemy, targetEnemyBase

class SmartAgent(base_agent.BaseAgent):
    def __init__(self):
        super(SmartAgent, self).__init__()

        self.qlearn = QLearningTable(actions=list(range(len(smart_actions))), load_qt= 'learningRushMarine4.csv')
        self.rewardTable = RewardCollector(tableName = 'learningRushMarine4Reward.csv')
    ...
    def reset(self):
        self.qlearn.save_qtable('learningRushMarine4.csv')
        self.rewardTable.collectReward(rewardRow = self.previous_score)
        self.rewardTable.save_table('learningRushMarine4Reward.csv')
        self.previous_score = 0
        self.episodes += 1
        self.previous_idle_workers = 0
        self.previous_workers_count = 12
