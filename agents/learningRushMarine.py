...
def get_state(obs):
    ...
    return (canTrainMarine, canTrainScv, scv_count, isArmyGrows, idle_workers, canSelectArmy, isEnemyVisible, canAttack, len(enemyX) ), (canSelectWorker, canGather, canBuildBarracks, canTrainScv, canBuildSupplyDepot, canTrainMarine), mineralTarget, targetTerranCenter, targetForBuild, targetScv, targetForBuildBaracks, targetBarracks, targetAttackEnemy, targetEnemyBase
...
class SmartAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        current_state = [state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7], state[8]]

        if self.previous_action is not None:
            reward = 0

            if smart_actions[int(self.previous_action)] == ACTION_TRAIN_MARINE and state[3]:
              reward += REWARD_BUILD_MARINE

            if smart_actions[int(self.previous_action)] == ACTION_BUILD_BARRACKS and state[4] < self.previous_idle_workers:
              reward += REWARD_BUILD_BARRACKS
            elif smart_actions[int(self.previous_action)] == ACTION_BUILD_SUPPLY_DEPOT and state[4] < self.previous_idle_workers:
              reward += REWARD_BUILD_BARRACKS
            elif state[4] < self.previous_idle_workers:
              reward += REWARD_SCV_BUSY

            if smart_actions[int(self.previous_action)] == ACTION_ATTACK_ENEMY and state[8] < self.previus_enemy_count:
              reward += REWARD_KILLED_ENEMY

            if smart_actions[int(self.previous_action)] == ACTION_TRAIN_SCV and state[2] >= self.previous_workers_count:
        ...
        self.previous_workers_count = state[2]
        self.previus_enemy_count = state[8]
