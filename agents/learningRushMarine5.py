...
def get_state(obs):
    ...
    hasIdleWorkders = 1 if idle_workers > 0 else 0
    canUseWorker = 1 if len(obs.observation.single_select) > 0 and obs.observation.single_select[0][0] == TERRAN_SCV else 0
    canBuildArmy = 1 if len(obs.observation.single_select) > 0 and obs.observation.single_select[0][0] == BARRACKS else 0
    ...
    hasBarracks = 1 if barracks_x.any() else 0
    hasSupply = 1 if supply_limit > 15 else 0
    ...
    if terran_center_x.any(): 
      targetForBuild = transformLocation(base_top_left, int(terran_center_x.mean()), 0 , int(terran_center_y.mean()), 20)                
    
    targetForBuildBaracks = None
    if terran_center_x.any(): 
      targetForBuildBaracks = transformLocation(base_top_left,int(terran_center_x.mean()), 20, int(terran_center_y.mean()), 0)
    ...
    return (isArmyGrows, hasIdleWorkders, canAttack, enemies_dead_count, hasArmy, hasBarracks, hasSupply, canUseWorker, canBuildArmy ), (canSelectWorker, canGather, canBuildBarracks, canTrainScv, canBuildSupplyDepot, canTrainMarine, canSelectArmy), mineralTarget, targetTerranCenter, targetForBuild, targetScv, targetForBuildBaracks, targetBarracks, targetAttackEnemy, targetEnemyBase

class SmartAgent(base_agent.BaseAgent):
    def __init__(self):
        super(SmartAgent, self).__init__()

        self.qlearn = QLearningTable(actions=list(range(len(smart_actions))), load_qt= 'learningRushMarine5.csv')
        self.rewardTable = RewardCollector(tableName = 'learningRushMarine5Reward.csv')
    ...
    def reset(self):
        self.qlearn.save_qtable('learningRushMarine5.csv')
        self.rewardTable.collectReward(rewardRow = self.previous_score)
        self.rewardTable.save_table('learningRushMarine5Reward.csv')
        self.previous_score = 0
        self.episodes += 1
        self.previous_idle_workers = 0
        self.previous_workers_count = 12
    ...
    def step(self, obs):
        ...
        current_state = [state[0], state[1], state[2], state[4], state[5], state[6], state[7], state[8]]
        ...
        if self.previous_action is not None:
            ...
            if  smart_actions[int(self.previous_action)] == ACTION_ATTACK_ENEMY and state[3] > self.previus_enemy_count:
        ...
        if smart_action == ACTION_DO_NOTHING or targetTerranCenter == None:
          ...
          if state[2] == 1 and state[4] == 1 and  targetAttackEnemy:
